...
def LYOUT_parse(path, tags):
    ...
    for line in lines:
        ...
        if TYPE:
            ...
            if CARD not in lyout[PLANT][PM]:
                lyout[PLANT][PM][CARD] = [{"TYPE": TYPE, "PM": PM, "CARD_NUM": int(
                    CARD)} for _ in range(lyout_sizes[TYPE])]
                CARD, TYPE = 0, 0
    ...
    for tag in tags:
        ...
        try:  # תכניס את התג
            ...
            if not tag_card == '0':
                lyout[PLANT][str(
                    tag_pm)][tag_card][tag_position]["NAME"] = tag["NAME"]
                lyout[PLANT][str(
                    tag_pm)][tag_card][tag_position]["STATUS"] = tag["STATUS"]
                lyout[PLANT][str(tag_pm)][tag_card][tag_position]["POINT_NUM"] = int(
                    tag["ID"].split("-")[-1])
                lyout[PLANT][str(
                    tag_pm)][tag_card][tag_position]["CARD_ID"] = tag["CARD_ID"]
        except Exception:
            print("\n\n\n---------======\n",
