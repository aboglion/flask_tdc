...
def main_parser(which_plant, which_pm='*', which_type='*'):
    ...
    PLANT_FOLDERS = glob.glob(f'{Consts.EB_FILES_DIR}/{Consts.PLANT_file_start[which_plant]}BC', recursive=True)
    if not PLANT_FOLDERS:
        print(f"{which_pm} !-->missed folder EC : {Consts.EB_FILES_DIR}/{Consts.PLANT_file_start[which_plant]}BC")
        return
    # כניסה ל  קבצים בתקייה
    for folder in PLANT_FOLDERS:
        # הוצאת שם המתקן הנוכחי מתוך השם של התקייה שלו
        current_plant = next((i for i in Consts.PLANT_file_start if Consts.PLANT_file_start[i] == folder[-4:-2]), None)
        ...
        tags_files = glob.glob(str(Path(folder) / f'{which_pm}{which_type}*.EB'))
        # אם אין קבצים EB
        if not (tags_files and ucn_file):
            print(f"{which_pm} !-->missed EB FILE: IN THER IS NO DATA FOR {which_pm} IN PLANT {current_plant}")
            return
        ...
        for index, file_path in enumerate(tags_files):
            ...
            if which_type == "*":
                ...
                current_type = file_name[4:6] if file_name[6] in [".", "_"] else file_name[4:7]
            else:
        ...
        try:
            ...
            else :print(f"err in db file of {current_plant}")
